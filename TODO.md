...
- 문제:
  - 홈 진입, `확인 필요`, `결과 보기`가 4000장 테스트에서 느림.
  - 현재는 summary 계산, 기존 앨범 backfill, 해외 기록, 최근 장소, 결과 그룹 렌더링, 썸네일 로딩이 한 흐름에 몰릴 수 있음.
- 목표:
  - 앱 진입은 저장된 summary/cache만 읽고 즉시 홈 표시.
  - 무거운 MediaStore 재스캔/backfill은 자동 실행을 최소화하거나 백그라운드로 지연.
  - 결과 화면은 그룹 summary부터 빠르게 표시하고 상세 렌더링은 필요할 때 나눠 처리.
- 구현 후보:
  - `loadRecentAlbumSummariesForUi()` 캐시 강화 및 invalidation 명확화.
  - 기존 앨범 backfill 자동 실행 조건 재점검. 필요 시 설정의 `발견 장소 다시 만들기` 중심으로 이동.
...
- 문제:
  - 결과 화면의 `정리될 앨범`을 누르면 대표사진만 보여서 사용자가 전체 사진을 확인하기 어려움.
- 목표:
  - 장소/앨범 그룹 클릭 시 앱 내부 상세 화면 표시.
  - 대표사진 1장이 아니라 해당 그룹 사진 그리드 제공.
  - 날짜 범위, 사진 수, `새 항목/이미 정리됨/위치 없음` 상태 표시.
- 구현 순서:
  - 성능 개선 후 진행.
  - 그룹 summary와 item list를 분리해, 상세 진입 시 필요한 사진만 lazy load.
...
- 확인:
  - 정리 중 notification update 로그가 항목마다 많이 발생함.
- 목표:
  - 알림 진행률은 0.5~1초 단위 또는 퍼센트 변화 기준으로 갱신.
  - 앱 내부 진행률은 현재처럼 빠르게 표시하되, 시스템 알림 부담을 줄임.
